...
def temp():
    corr_matrix = df_clean[pollutants + ['AQI指数']].corr().round(2)
    print(corr_matrix)

    rules = pd.read_csv('work1/关联规则_强规则.csv')
    severe = rules[rules['consequents'].str.contains('严重污染|重度污染')]
    print(f"共{len(severe)}条")
    print(severe.head(15).to_string())
